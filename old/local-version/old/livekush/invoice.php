...
<td width="50%" style="color:#0000BB;"><span style="font-weight: bold; font-size: 14pt;">
	Shivam Dressess
</span><br />Opp, Post Office<br />Gheekanta Road , <br />Ahmedabad<br /><span style="font-size: 15pt;">&#9742;</span> 079-65235816</td>
...
<thead>
    ...
    <?php
		if(count($stdm[$i]) > 0) {
			...
			<td align="center">'. $count = count($stdm[$i]) .'</td>
		...
		if(count($stdf[$i]) > 0) {
			...
			<td align="center">'. $count = count($stdf[$i]) .'</td>
			<td>PENTS / SALWAR READY GARMENT </td>
			<td align="right">'.$cost.'</td>';
			$invoiceData .= '<td align="right">';
			$price = ($count * $cost) ;
	...
	$invoiceData .= '<tr>
					<td colspan="4" class="blanktotal"> </td>
